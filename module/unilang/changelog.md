...
- Added `get_string_normalized< 'a >( &'a self, name : &str ) -> Option< &'a str >` — trims whitespace, returns `Some("")` for whitespace-only (no heap allocation — borrows from self)
- Added `require_string_normalized< 'a >( &'a self, name : &str ) -> Result< &'a str, Error >` — trims required args with same semantics
- Upgraded internal deps: former ~2.42.0 (was ~2.40.0), mod_interface ~0.58.0, strs_tools ~0.43.0, error_tools ~0.37.0

**Migration guide:**
- Old: `cmd.get_string("x").map(|s| s.trim())`
- New: `cmd.get_string_normalized("x")`
- Note: whitespace-only returns `Some("")`; add `.filter(|s| !s.is_empty())` if non-empty value required
- Old (double-trim bug): `cmd.get_string("x").filter(|s| !s.trim().is_empty())`
- New: `cmd.get_string_normalized("x").filter(|s| !s.is_empty())`
