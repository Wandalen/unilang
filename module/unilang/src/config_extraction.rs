...
mod private
{
  ...
  /// Look up `key` in `config` and apply `f` to the raw JSON value.
  ///
  /// Centralizes the repeated `config.get(key).and_then(|(v, _)| f(v))` pattern
  /// shared by every type-specific extractor below.
  #[ inline ]
  fn extract_with< S, T, F >( config : &ConfigMap< S >, key : &str, f : F ) -> Option< T >
  where
    F : FnOnce( &JsonValue ) -> Option< T >,
  {
    config.get( key ).and_then( |( v, _ )| f( v ) )
  }

  /// Extract `u8` value from config.
  ///
  /// Returns `None` if key is missing, value is not a number, or value overflows `u8`.
  #[ inline ]
  #[ must_use ]
  pub fn extract_u8< S >( config : &ConfigMap< S >, key : &str ) -> Option< u8 >
  {
    extract_with( config, key, |v| v.as_u64().and_then( |n| u8::try_from( n ).ok() ) )
  }

  /// Extract `u16` value from config.
  ///
  /// Returns `None` if key is missing, value is not a number, or value overflows `u16`.
  #[ inline ]
  #[ must_use ]
  pub fn extract_u16< S >( config : &ConfigMap< S >, key : &str ) -> Option< u16 >
  {
    extract_with( config, key, |v| v.as_u64().and_then( |n| u16::try_from( n ).ok() ) )
  }

  /// Extract `u32` value from config.
  ///
  /// Returns `None` if key is missing, value is not a number, or value overflows `u32`.
  #[ inline ]
  #[ must_use ]
  pub fn extract_u32< S >( config : &ConfigMap< S >, key : &str ) -> Option< u32 >
  {
    extract_with( config, key, |v| v.as_u64().and_then( |n| u32::try_from( n ).ok() ) )
  }

  /// Extract `u64` value from config.
  ///
  /// Returns `None` if key is missing or value is not a number.
  #[ inline ]
  #[ must_use ]
  pub fn extract_u64< S >( config : &ConfigMap< S >, key : &str ) -> Option< u64 >
  {
    extract_with( config, key, |v| v.as_u64() )
  }

  /// Extract `i32` value from config.
  ///
  /// Returns `None` if key is missing, value is not a number, or value overflows `i32`.
  #[ inline ]
  #[ must_use ]
  pub fn extract_i32< S >( config : &ConfigMap< S >, key : &str ) -> Option< i32 >
  {
    extract_with( config, key, |v| v.as_i64().and_then( |n| i32::try_from( n ).ok() ) )
  }
  ...
  {
    extract_with( config, key, |v| v.as_i64() )
  }
  ...
  {
    extract_with( config, key, |v| v.as_f64() )
  }

  /// Extract `bool` value from config.
  ///
  /// Returns `None` if key is missing or value is not a boolean.
  #[ inline ]
  #[ must_use ]
  pub fn extract_bool< S >( config : &ConfigMap< S >, key : &str ) -> Option< bool >
  {
    extract_with( config, key, |v| v.as_bool() )
  }

  /// Extract `String` value from config.
  ///
  /// Returns `None` if key is missing, value is `null`, or value is not a string.
  #[ inline ]
  #[ must_use ]
  pub fn extract_string< S >( config : &ConfigMap< S >, key : &str ) -> Option< String >
  {
    extract_with( config, key, |v| if v.is_null() { None } else { v.as_str().map( str::to_string ) } )
  }

  /// Extract array of strings from config. Non-string elements are filtered out.
  ///
  /// Returns `None` if key is missing or value is not an array.
  #[ inline ]
  #[ must_use ]
  pub fn extract_string_array< S >( config : &ConfigMap< S >, key : &str ) -> Option< Vec< String > >
  {
    extract_with( config, key, |v| v.as_array().map( |arr| arr.iter().filter_map( |e| e.as_str().map( String::from ) ).collect() ) )
  }
}
