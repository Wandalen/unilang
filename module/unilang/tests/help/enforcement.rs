...
fn test_mandatory_help_counterpart_generation()
{
  ...
  let result = registry.register_with_routine( &cmd, Box::new( test_routine ) );
  ...
  assert_eq!( help_cmd.name().as_str(), ".test_mandatory.help" );
...
fn test_mandatory_help_for_all_registration_methods()
{
  ...
  // Test 1: register_with_routine method
  let cmd1 = CommandDefinition::former()
    .name( ".test_runtime" )
    .description( "Runtime registration test" )
    .end();

  registry.register_with_routine( &cmd1, Box::new( test_routine ) ).unwrap();
  ...
  registry.register( cmd2 ).expect( "Registration should succeed" );
  ...
  // happens in register_with_routine which is the primary registration method

  // Test 3: register_with_auto_help (should behave identically to register_with_routine)
...
fn test_mandatory_global_help_command()
{
  ...
  assert!( registry.routine( ".help" ).is_some(), "Global .help routine MUST be available" );

  // Verify global help command properties
  let global_help = registry.command( ".help" ).unwrap();
  assert_eq!( global_help.name().as_str(), ".help" );
...
#[ test ]
fn test_help_generated_by_default()
{
  let mut registry = CommandRegistry::new();

  // Create command and register it using defaults (auto_help_enabled defaults to true)
  let cmd = CommandDefinition::former()
    .name( ".test_no_flexibility" )
    .description( "Test that help is generated by default" )
    .end();

  registry.register_with_routine( &cmd, Box::new( test_routine ) ).unwrap();

  // Help MUST be generated by default
  assert!( registry.command( ".test_no_flexibility.help" ).is_some(),
           "Help MUST be generated by default (auto_help_enabled defaults to true)" );

  println!( "✅ Help is generated by default" );
}
...
fn test_mandatory_help_with_complex_commands()
{
  ...
  registry.register_with_routine( &complex_cmd, Box::new( test_routine ) ).unwrap();
  ...
  let help_text = registry.help_for_command( ".testing.complex_test" ).unwrap();
...
fn test_help_command_execution_mandatory()
{
  ...
  let help_routine = registry.routine( ".test_execution.help" ).unwrap();
...
fn test_global_help_execution()
{
  ...
  let global_help_routine = registry.routine( ".help" ).unwrap();
...
fn test_help_enforcement_edge_cases()
{
  ...
  // Test 1: Empty name is rejected at the type level
  // The builder panics at .end() for invalid names, so we test CommandName directly
  assert!( CommandName::new( "" ).is_err(), "Empty name should be rejected by CommandName" );

  // Test 2: Name without dot prefix is rejected at the type level
  assert!( CommandName::new( "no_dot" ).is_err(), "Name without dot prefix should be rejected by CommandName" );
  ...
  registry.register_with_routine( &duplicate_cmd, Box::new( test_routine ) ).unwrap();
  let result = registry.register_with_routine( &duplicate_cmd, Box::new( test_routine ) );
...
fn test_help_content_comprehensive()
{
  ...
  registry.register_with_routine( &comprehensive_cmd, Box::new( test_routine ) ).unwrap();
