...
*   `get_string_normalized<'a>(&'a self, name: &str) -> Option<&'a str>` - Extracts optional string, trimming leading/trailing Unicode whitespace. Returns `Some("")` for whitespace-only input (callers filter with `.filter(|s| !s.is_empty())`). No allocation — borrows from self.
*   `require_string_normalized<'a>(&'a self, name: &str) -> Result<&'a str, Error>` - Extracts required string, trimming whitespace. Same trim semantics as `get_string_normalized()`. Returns error if argument is missing or wrong type.
